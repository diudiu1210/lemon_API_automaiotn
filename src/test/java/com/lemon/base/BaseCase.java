...
//        RestAssured.filters(new RequestLoggingFilter(fileOutPut),new ResponseLoggingFilter(fileOutPut));
    ...
    public List<ExcelData> getData(int index,int totalSheet){
        File file = new File(Constants.V3_CASE);
    ...
    public static void paramsV3Config() {
        /*
        生成v3版本必要的请求参数
        */
        if (Constants.IS_V3) {
            ...
            //将timestamp 加入环境变量
            GlobalEnvironment.envVariable.put("timestamp", timestamp);
            String interceptStr = GlobalEnvironment.envVariable.get("investor_token").toString().substring(0, 50);
            String tempStr = interceptStr + timestamp;
            String sign = StringEscapeUtils.escapeJava(EncryptUtils.rsaEncrypt(tempStr));
            GlobalEnvironment.envVariable.put("sign",sign);
            System.out.println("加密后的字符串：：" + sign);
        }
    }

    public static void main(String[] args) {
        String a = "{\"code\":\"a" + "\r" + "/b/c\"}";
        String b = StringEscapeUtils.unescapeJavaScript(a);
        String c = StringEscapeUtils.escapeJava(a);
